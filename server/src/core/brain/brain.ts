...
export default class Brain {
  ...
  public talk(rawSpeech: string, end = false): void {
    ...
    if (rawSpeech !== '') {
      ...
      SOCKET_SERVER.sendSocketMessage('answer', rawSpeech)
    }
  }
  ...
  private handleAskToRepeat(nluResult: NLUResult): void {
    if (!this.isMuted) {
      ...
      SOCKET_SERVER.sendSocketMessage('ask-to-repeat', nluResult)
    }
  }
  ...
  private speakSkillError(): void {
    ...
    if (!this.isMuted) {
      this.talk(speech)
      SOCKET_SERVER.sendSocketMessage('is-typing', false)
    }
    this.speeches.push(speech)
  }
        ...
        if (actionType === SkillActionType.Logic) {
          ...
          this.skillProcess?.stdout.on('end', () => {
            ...
            if (
              ...
              SOCKET_SERVER.sendSocketMessage('suggest', nextAction.suggestions)
            }
            if (
              action?.suggestions &&
              skillResult?.output.core?.showSuggestions
            ) {
              SOCKET_SERVER.sendSocketMessage('suggest', action.suggestions)
            }
