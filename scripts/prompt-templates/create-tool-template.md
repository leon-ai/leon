...
- Fill the `tools/{TOOL_TOOLKIT_NAME}/{TOOL_NAME}/tool.json` file. Keep `description` to a concise routing summary. Put shared operational instructions in tool-level `progressive_guidance`, and function-specific instructions in each function's optional `progressive_guidance`. In progressive mode, tools with function guidance expose short function summaries first; selected schemas and guidance load together before calls are constructed. Keep routing and safety principles shared, not hidden behind a function choice. You must provide the binaries, resources, and function definitions by following the OpenAI function-calling standard. Create the file if it does not exist yet.
