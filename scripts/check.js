    ...
    try {
      ...
      try {
        const p = await command(
          "pipenv run python bridges/python/main.py scripts/assets/query-object.json",
          { shell: true }
        );
        log.info(p.command);
        log.success(`${p.stdout}\n`);
      ...
      if (
        !fs.existsSync(nlpModelPath) ||
        !Object.keys(fs.readFileSync(nlpModelPath)).length
      ) {
        ...
        log.error(
          'NLP model not found or broken. Try to generate a new one: "npm run train"\n'
        );
      } else {
