...
# ARCHITECTURE
- Generated at: 2026-07-14T17:35:46+08:00
...
- A `<profile>:<token>` credential selects and authenticates the profile across Socket.IO clients, HTTP integrations, and Leon Link.
## Leon Link
- Leon Link is an optional process on a user device that connects its enabled and available profile tools to a remote Leon server.
- Eligible tool calls are routed through Link and executed on that device; those tools become unavailable when Link disconnects.
- Link provides generic transport only. Device-, application-, and company-specific behavior stays in tools and skills instead of Leon Core.
## Agent Loop
